...
interface LegacyPluginDetailPageProps {
  params: Promise<{ pluginId: string }>;
}

export default function LegacyPluginDetailPage({ params }: LegacyPluginDetailPageProps) {
  ...
  useEffect(() => {
    async function loadAndRedirect() {
      ...
      if (targetId) {
        router.replace(`/${targetId}/plugins/${encodeURIComponent(pluginId)}`);
      } else {
        router.replace("/chat");
      }
    }
    ...
    loadAndRedirect();
  }, [router, pluginId, currentWorkspaceId, setWorkspaces, setCurrentWorkspace]);
